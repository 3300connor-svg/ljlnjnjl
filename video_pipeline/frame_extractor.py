...
_CASCADE_EYE     = str(_HERE / "haarcascade_eye.xml")
_CASCADE_SMILE   = str(_HERE / "haarcascade_smile.xml")
...
_CASCADE_URLS = {
    ...
    _CASCADE_EYE:     "https://raw.githubusercontent.com/opencv/opencv/master/data/haarcascades/haarcascade_eye.xml",
    _CASCADE_SMILE:   "https://raw.githubusercontent.com/opencv/opencv/master/data/haarcascades/haarcascade_smile.xml",
}
                ...
                if len(faces) > 0:
                    for fx, fy, fw, fh in faces:
                        ...
                        if area > best_face_area:
                            best_face_area = area
                            best_face_box  = (fx, fy, fw, fh)
                            face_center_y  = (fy + fh / 2) / h

        if best_face_area > 0:
            fx, fy, fw, fh = best_face_box
            ...
            score = sharpness * (1 + face_ratio * 20) * position_bonus

            if face_roi.size > 0:
                # Eyes open: loosened minNeighbors so more frames get detected,
                # stronger blink penalty so missed-eye frames are firmly rejected
                eye_cc = cv2.CascadeClassifier(_CASCADE_EYE)
                if not eye_cc.empty():
                    ...
                    if len(eyes) >= 2:
                        ...
                        score *= 0.45  # strong blink penalty

                # Open mouth: darkness ratio in lower 35% of face ROI.
                # Open mouths create a dark cavity (tongue/throat) vs uniform skin.
                # More reliable than smile cascade on filtered/made-up Instagram content.
                lower_face = face_roi[int(face_roi.shape[0] * 0.65):, :]
                if lower_face.size > 0:
                    dark_ratio = float(np.sum(lower_face < 70)) / lower_face.size
                    if dark_ratio > 0.06:
                        score *= 0.6  # open mouth detected — prefer neutral
...
def extract_best_frame(video_path: str, output_path: str, n_candidates: int = 36) -> str:
    """
