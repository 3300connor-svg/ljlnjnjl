...
from config import (
    ...
    KLING_STD_MODEL, KLING_PRO_MODEL,
...
def edit_message_kb(msg_id: int, text: str, kb: dict, parse_mode: str = "HTML"):
    if not msg_id:
        return
    _tg("editMessageText", chat_id=TELEGRAM_CHAT_ID, message_id=msg_id,
...
def send_video(path: str, caption: str = ""):
    ...
    if size > MAX_TG_VIDEO_BYTES:
        send(f"⚠️ Output is {size // 1024 // 1024}MB — over Telegram's 50MB limit. Find it in outputs/videos/.")
        return
...
_KB_KLING_TIER = {"inline_keyboard": [
    [
        {"text": "⚡  STANDARD", "callback_data": "kling_std"},
        {"text": "🔥  PRO",      "callback_data": "kling_pro"},
        {"text": "❌  CANCEL",   "callback_data": "cancel"},
    ],
]}
...
def _get_balance() -> float:
    try:
        r = requests.get(
            f"{WAVESPEED_BASE}/api/v3/balance",
...
def _kling_cost_str(duration_s: float, tier: str = "std") -> str:
    rate = 0.126 if tier == "std" else 0.252
    return f"~${min(duration_s, 30) * rate:.2f}"


# ── Main pipeline ─────────────────────────────────────────────────────────────

def _run_pipeline(local_path: str, label: str, cancel_event: threading.Event, source_url: str = "", msg_id: int = 0):
    ...
    if msg_id:
        edit_message(msg_id, "⏳ Extracting best frame...")
    else:
        msg_id = send_with_id("⏳ Extracting best frame...")
    ...
    try:
        ...
        _ref_str = source_url or label[:60]

        # Seal the download tracker before sending new messages below it
        edit_message(msg_id, "✅ Downloaded · frame extracted")

        _caption = (
            f"1. Best frame\n"
            f"2. Reference Image (Face)\n\n"
            f"{_ref_str}  ·  {_dur_str}\n\n"
            f"Prompt:\n{DEFAULT_IMAGE_PROMPT}"
        )
        _media = json.dumps([
            {"type": "photo", "media": "attach://frame", "caption": _caption},
        ...
        try:
            ...
            if not _r.json().get("result"):
                print(f"[pipeline] sendMediaGroup failed: {_r.text[:200]}")
        except Exception as e:
            print(f"[pipeline] sendMediaGroup error: {e}")

        _bal_swap = _get_balance()
        _bal_swap_str = f"${_bal_swap:.3f}" if _bal_swap >= 0 else "?"

        # Ask for prompt via force_reply
        r_fp = _tg("sendMessage",
            chat_id=TELEGRAM_CHAT_ID,
            text="✏️ Type your prompt:",
            reply_markup={"force_reply": True, "input_field_placeholder": "Describe what you want..."})
        _log_msg_id(r_fp.get("result", {}).get("message_id", 0))

        seedream_prompt = _wait_reply(timeout=300).strip()
        if not seedream_prompt or seedream_prompt.lower() == "cancel":
            send("❌ Cancelled.")
            raise _PipelineCancelled()

        # Confirm with GO / CANCEL
        msg_id = send_with_kb(
            f"Your prompt:\n{seedream_prompt}\n\n💰 Seedream 4.5: $0.04  ·  Balance: {_bal_swap_str}",
            _KB_GO_CANCEL,
        )
        action = _wait_reply(timeout=180)
        if not action or action.strip().lower() == "cancel":
            send("❌ Cancelled.")
            raise _PipelineCancelled()

        # ── Step 2: Upload video + frame for WaveSpeed ────────────────────────
        _check()
        edit_message(msg_id, "⏳ Uploading video...")
        try:
            video_ws_url = upload_for_wavespeed(local_path, progress_cb=lambda m: edit_message(msg_id, f"⏳ {m}"))
        ...
        _check()
        edit_message(msg_id, "⏳ Uploading frame...")
        try:
            frame_ws_url = upload_for_wavespeed(frame_path, progress_cb=lambda m: edit_message(msg_id, f"⏳ {m}"))
        ...
        while True:
            _check()
            edit_message(msg_id, "⏳ Generating face + hair swap...")
        ...
        if up_choice.strip().lower() == "upscale_2x":
            try:
                ...
                edit_message(msg_id, "⏳ Upscaling with Real-ESRGAN...")
                _tmp_url = upload_for_wavespeed(swap_path, progress_cb=lambda m: edit_message(msg_id, f"⏳ {m}"))
                with _typing():
        ...
        edit_message(msg_id, "⏳ Uploading swap image...")
        try:
            swap_ws_url = upload_for_wavespeed(swap_path, progress_cb=lambda m: edit_message(msg_id, f"⏳ {m}"))
        ...
        _check()
        edit_message(msg_id, "⏳ Trimming video...")
        try:
        ...
        if was_trimmed:
            _check()
            edit_message(msg_id, "⏳ Re-uploading trimmed video...")
            try:
                video_ws_url = upload_for_wavespeed(trimmed_path, progress_cb=lambda m: edit_message(msg_id, f"⏳ {m}"))
            except Exception as e:
                send(f"❌ Trimmed video upload failed: {str(e)[:300]}")
                return

        # ── Step 6: Kling tier selection then generate ────────────────────────
        _check()
        try:
        ...
        _std_cost   = _kling_cost_str(actual_dur, "std")
        _pro_cost   = _kling_cost_str(actual_dur, "pro")
        ...
        tier_reply = _wait_reply(timeout=120).strip().lower()

        if not tier_reply or tier_reply == "cancel":
            send("❌ Kling cancelled.")
            raise _PipelineCancelled()

        if tier_reply == "kling_std":
            kling_model = KLING_STD_MODEL
            tier_label  = "Standard"
        else:
            kling_model = KLING_PRO_MODEL
            tier_label  = "Pro"

        kling        = KlingMotionClient(WAVESPEED_API_KEY, model=kling_model)
        kling_prompt = DEFAULT_KLING_PROMPT

        while True:
            _check()
            edit_message(msg_id, f"⏳ Submitting to Kling 3.0 {tier_label}...")
            try:
                with _typing():
        ...
        _check()
        edit_message(msg_id, "⏳ Fetching trending songs...")
        audio_name = None
        try:
        ...
        _check()
        edit_message(msg_id, "⏳ Scoring virality...")
        score_text = ""
        try:
            from virality_ranker import rank_videos
        ...
        # ── Done ──────────────────────────────────────────────────────────────
        edit_message(msg_id, "✅ Pipeline complete!")
...
def _process_single(url: str):
    ...
    try:
        ...
        if cancel_event.is_set():
            edit_message(msg_id, "🛑 Cancelled before pipeline started.")
            return

        _run_pipeline(raw_path, label, cancel_event, source_url=url, msg_id=msg_id)
