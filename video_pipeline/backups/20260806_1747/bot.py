...
def send_photo_file(path: str, caption: str = "") -> dict:
    with open(path, "rb") as f:
        try:
            r = requests.post(
                f"{TG_BASE}/sendPhoto",
            ...
            data = r.json()
            _log_msg_id(data.get("result", {}).get("message_id", 0))
            return data
        except Exception as e:
...
def _run_pipeline(local_path: str, label: str, cancel_event: threading.Event, source_url: str = ""):
    ...
    try:
        ...
        _caption = (
            ...
            f"{_link}  ·  {_dur_str}\n\n"
            f"<tg-spoiler>Prompt:\n{DEFAULT_IMAGE_PROMPT}</tg-spoiler>"
        )
        ...
        # Force-reply prompt — keyboard pops open automatically
        _fr_r = _tg(
            "sendMessage",
            chat_id=TELEGRAM_CHAT_ID,
            text="Reply <b>go</b> to run · <b>cancel</b> to abort · or type a custom prompt",
            parse_mode="HTML",
            reply_markup={"force_reply": True, "input_field_placeholder": "go / cancel / custom prompt..."},
        )
        _log_msg_id(_fr_r.get("result", {}).get("message_id", 0))
        ...
        if not action or action_lower == "cancel":
            send("❌ Cancelled.")
            raise _PipelineCancelled()
        seedream_prompt = DEFAULT_IMAGE_PROMPT if action_lower == "go" else action.strip()
        ...
        while True:
            _check()
            edit_message(msg_id, _step_text(3, "generating face swap..."))
            try:
                seedream.generate(
                    current_prompt,
                    FACE_REFERENCE_URL,
                ...
                send(f"❌ WaveSpeed API error: {str(e)[:300]}\n\nReply 'retry' to try again or 'cancel' to abort.")
            ...
            send_photo_file(swap_path, caption=(
                f"🎭 Face swap result\n\n"
                f"Reply:\n"
                f"  'go'     → accept, continue to Kling\n"
                f"  'retry'  → regenerate same prompt\n"
                f"  'cancel' → abort\n"
                f"  anything else → regen with that as custom prompt"
            ))
            action = _wait_reply(timeout=180)
            action_lower = action.strip().lower()
        ...
        # ── Step 4: Upload swap image + trim video ────────────────────────────
        _check()
        ...
        try:
            ...
            ws_balance = f"${bal_r.json().get('data', {}).get('balance', '?')}"
        except Exception:
            pass
        ...
        while True:
            _check()
            send(
                f"🎬 Kling 3.0 Motion Control — confirm to run\n\n"
                f"🖼️  Image:    hair swap (just generated)\n"
                f"🎥  Video:    {label[:40]}{' ✂️ trimmed' if was_trimmed else ''}\n"
                f"⏱️  Duration: {actual_dur:.1f}s\n"
                f"💰  Cost:     {actual_cost}   (Balance: {ws_balance})\n"
                f"🔊  Sound:    preserved\n"
                f"📐  Mode:     motion control / video orientation"
                f"{virality_preview}\n\n"
                f"Reply:\n"
                f"  'go'     → run Kling now\n"
                f"  'cancel' → abort\n"
                f"  anything else → used as Kling motion prompt"
            )
            ...
            kling_prompt = "" if reply_lower == "go" else reply.strip()
            ...
            try:
                ...
                send(f"❌ Kling API error: {str(e)[:300]}\n\nReply 'retry' to try again or 'cancel' to abort.")
        ...
        send_video(out_path, caption=caption)
